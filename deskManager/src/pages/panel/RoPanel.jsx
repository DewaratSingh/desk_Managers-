...
export default function RoPanel({ releaseOrder, purchaseOrder, tradeId }) {
  // Prompt card when PO exists but no RO yet
  if (!releaseOrder) {
    if (!purchaseOrder) return null;
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-8 text-center shadow-sm space-y-4">
        <div className="max-w-sm mx-auto space-y-1">
          <h3 className="text-sm font-bold text-slate-800">No Release Order Raised</h3>
          <p className="text-xs text-slate-500 font-semibold">
            A Purchase Order has been registered. The next step is to raise a Release Order.
          </p>
        </div>
        <Link
          to={`/addReleaseOrder?trade_id=${tradeId}`}
        ...
        >
          <Plus size={13} /> Create Release Order
        </Link>
      </div>
    );
  }
  ...
  const packing    = parseFloat(releaseOrder.packing_forward) || 0;
  const other      = parseFloat(releaseOrder.other)           || 0;
  const basicVal   = parseFloat(releaseOrder.basic_value)     || 0;
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
      ...
      <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            to={`/release-order/${encodeURIComponent(releaseOrder.ro_no)}`}
          ...
          <Link
            to={`/updateReleaseOrder/${releaseOrder.ro_no}?trade_id=${tradeId}`}
            className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-bold text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer"
          >
            <Edit2 size={10} /> Edit
          </Link>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pb-5 border-b border-slate-100">
          {[
            { label: 'RO Date',       value: fmtDate(releaseOrder.ro_date) },
            { label: 'Delivery Date', value: fmtDate(releaseOrder.delivery_date) },
            { label: 'Contract Ref', value: releaseOrder.contract_ref || '—' },
