...
export default function ReleaseOrderView() {
  ...
  const fetchRo = async () => {
    ...
    try {
      const res = await fetch(`/api/release-orders/${encodeURIComponent(ro_no)}`);
      if (!res.ok) throw new Error('Release order not found');
  ...
  const saveEdit = async (itemCode) => {
    ...
    try {
      const res = await fetch(
        `/api/release-orders/${encodeURIComponent(ro_no)}/items/${encodeURIComponent(itemCode)}`,
        {
          method: 'PUT',
      ...
      setRo(prev => ({ ...prev, items: updatedItems }));
      setEditingCode(null);
  ...
  /* Totals */
  const itemsBasic = (ro?.items || []).reduce((s, i) =>
    s + (parseFloat(i.unit_price)||0) * (parseInt(i.quantity)||0), 0);
  const gstTotal = (ro?.items || []).reduce((s, i) => {
    const line = (parseFloat(i.unit_price)||0) * (parseInt(i.quantity)||0);
    return s + line * ((parseFloat(i.gst_rate)||0) / 100);
  }, 0);
  const transport  = parseFloat(ro?.transport)       || 0;
  const packing    = parseFloat(ro?.packing_forward) || 0;
  const other      = parseFloat(ro?.other)           || 0;
  const basicVal   = parseFloat(ro?.basic_value)     || 0;
  const grandTotal = itemsBasic + gstTotal + transport + packing + other + basicVal;

  /* Loading / Error */
  if (isLoading) return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center">
      <p className="text-slate-500 font-bold text-sm animate-pulse">Loading Release Order…</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 print:bg-white">
        ...
        <div className="w-full px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 min-w-0">
              ...
              <span className="font-black text-slate-800 truncate">{ro.ro_no}</span>
            </div>
          </div>
      ...
      <div className="w-full px-4 sm:px-8 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-slate-900">Release Order</h1>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          <div className="bg-slate-50 px-6 py-3 border-b border-slate-200">
            <span className="text-xs font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
              <Hash size={13} style={{ color: 'var(--theme-color)' }} /> RO Information
            </span>
          </div>
          <div className="p-6 grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-4">
            {[
              { label: 'RO No.',        value: ro.ro_no,            mono: true },
              { label: 'RO Date',       value: fmtDate(ro.ro_date) },
              { label: 'Delivery Date', value: fmtDate(ro.delivery_date) },
              { label: 'Contract Ref',  value: ro.contract_ref || '—' },
              { label: 'Buyer',         value: ro.buyer_name   || '—' },
              { label: 'Seller/Cust',   value: ro.customer_name || '—' },
              { label: 'Trade',         value: ro.trade_id     || '—', mono: true },
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="mt-5 pt-4 border-t-2 border-slate-200 flex justify-between items-center">
              <span className="text-xs font-extrabold text-slate-500 uppercase tracking-wider">Grand Total RO Value</span>
        ...
        <div className="flex items-center justify-end gap-3 print:hidden pb-4">
          <Link
            to={`/updateReleaseOrder/${ro.ro_no}?trade_id=${ro.trade_id}`}
          ...
          >
            <Edit2 size={15} /> Edit Release Order
          </Link>
          {ro.trade_id && (
            <Link to={`/trade/${ro.trade_id}`}
              className="px-5 py-2.5 border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold rounded-lg text-sm transition-colors cursor-pointer flex items-center gap-1.5">
